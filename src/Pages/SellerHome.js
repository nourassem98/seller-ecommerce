...
export default class SellerHome extends Component {
    ...
    render() {
        ...
        return (
            <>
                <header style={{ backgroundColor: "rgba(149, 157, 165, 0.3)" }}>
                    <Row>
                        <Col md={10} style={{ textAlign: "center", marginLeft: "5%" }}>
                            <h4>Welcome Back, {seller.name} </h4>
                        </Col>
                        <Col md={1}>
                            <Button className='logoutBtn' onClick={() => { sessionStorage.clear(); window.location.replace("/") }}> Logout </Button>
                        </Col>
                    </Row>
                </header>
